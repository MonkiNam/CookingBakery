...
namespace EventManagementFPT.Modules.ReportModule
{
    public class ReportService : IReportService
    {
        ...
        {
            var report = await _reportRepository.GetFirstOrDefaultAsync(x => x.ReportId.Equals(reportId) && x.Status != ReportEnum.Processed);
        ...
        public async Task DeleteReport(Guid? id)
        {
            ...
            reportDelete.Status = ReportEnum.Processed;
            await _reportRepository.UpdateAsync(reportDelete);
        }
        public async Task<Report> GetReportByID(Guid? reportId)
        {
            return await _reportRepository.GetFirstOrDefaultAsync(x => x.ReportId.Equals(reportId));
        }
    }
}
